...
namespace op
{
	...
	void addKeypointsToText(TextOfstream& textOfstream,
		const std::vector<std::pair<Array<float>, std::string>>& keypointVector)
	{
		try
		{
			...
			for (auto person = 0; person < numberPeople; person++)
			{
				...
				const auto& keypoints = keypointVector[0].first;
				const auto& keypointName = keypointVector[0].second;
				const auto numberElementsPerRaw = keypoints.getSize(1) * keypoints.getSize(2);
				...
				if (numberElementsPerRaw > 0)
				{
					...
					for (auto element = 0; element < numberElementsPerRaw; element += 3)
					{
						totalScore = totalScore + keypoints[finalIndex + element + 2];

					}
					if (totalScore / 18 >= 0.5)
					{
						
						for (auto element = 0; element < numberElementsPerRaw; element++)
						{
							textOfstream.plainText(keypoints[finalIndex + element]);
							textOfstream.comma();
						}
	...
	void savePeopleText(const Array<float>& keypoints,
		const std::vector<std::vector<std::array<float, 3>>>& candidates,
		const std::string& keypointName, const std::string& fileName,
		const bool humanReadable)
	{
	...
	OP_API void savePeopleText(const std::vector<std::pair<Array<float>, std::string>>& keypointVector, const std::vector<std::vector<std::array<float, 3>>>& candidates, const std::string & fileName, const bool humanReadable)
	{
		try
		{
			// Security checks
			for (const auto& keypointPair : keypointVector)
				if (!keypointPair.first.empty() && keypointPair.first.getNumberDimensions() != 3)
					error("keypointVector.getNumberDimensions() != 3.", __LINE__, __FUNCTION__, __FILE__);
			// Record frame on desired path
			TextOfstream textOfstream{ fileName, humanReadable };
			
			//给每行数据开头添加帧数  
			const auto& keypointName = keypointVector[0].second;
			textOfstream.plainText(keypointName);
			textOfstream.comma();

			addKeypointsToText(textOfstream, keypointVector);
			//给每行数据结尾添加换行
			textOfstream.huiche();
		}
		catch (const std::exception& e)
